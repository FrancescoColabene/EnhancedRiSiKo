...
void main() {
	...
	vec3 DiffSpec = BRDF(V, N, L, MD, MS, ubo.fresnel, ubo.metallic, ubo.roughness);
	
	vec3 Ambient = MD * 0.05f;
	
	outColor = vec4(clamp(0.95 * DiffSpec * gubo.DlightColor.rgb + Ambient,0.0,1.0), 1.0f);
}